...
export function ScrollExpansionHero({
  ...
  const mediaHeight = useTransform(scrollYProgress, [0, 0.7], ["340px", "76vh"])
  ...
  const word1X = useTransform(scrollYProgress, [0, 0.65], ["0%", "-38%"])
  const word2X = useTransform(scrollYProgress, [0, 0.65], ["0%",  "38%"])
  ...
  const carouselOpacity = useTransform(scrollYProgress, [0.72, 0.92], [0, 1])
  const carouselY       = useTransform(scrollYProgress, [0.72, 0.92], [40, 0])
  ...
  return (
    // Outer gives the scroll distance; inner is sticky
    <div ref={outerRef} style={{ minHeight: "250vh" }}>
      ...
      >
        {/* ── Ambient glow background (fades out as media expands) ── */}
        <motion.div
          style={{
            position: "absolute",
            inset: 0,
      ...
      {/* ── Carousel appears after the sticky zone ─────────────────── */}
      <motion.div
        style={{
